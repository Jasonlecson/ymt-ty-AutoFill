...
//补光灯控制模式 0:自动  1:手动
void FlashSetFillLightCtrlMode(uint16_t data)
{
	u16 ReadDat = data;
  STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_CONTROL,&ReadDat,1);
}
//补光灯控制模式 0:自动  1:手动
u16 FlashGetFillLightCtrlMode(void)
{
	u16 ReadDat = 0;
  STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_CONTROL,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}
//补光灯颜色
void FlashSetFillLightColor(uint16_t data)
{
	u16 ReadDat = data;
  STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_COLOR,&ReadDat,1);
}
//补光灯颜色
u16 FlashGetFillLightColor(void)
{
	u16 ReadDat = 0;
  STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_COLOR,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}
//补光灯喜好
void FlashSetFillLightHabit(uint16_t data)
{
	u16 ReadDat = data;
  STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_HABIT,&ReadDat,1);
}
//补光灯喜好
u16 FlashGetFillLightHabit(void)
{
	u16 ReadDat = 0;
  STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_HABIT,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}

void FlashSetFillLightTimer(uint16_t data)
{
	u16 ReadDat = data;
  STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_TIMER,&ReadDat,1);
}

u16 FlashGetFillLightTimer(void)
{
	u16 ReadDat = 0;
  STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_TIMER,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}
void FlashSetFillLightBright(int value)
{
	u16 data;
	data = value;
  STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_BRIGHT,&data,1);
}

int FlashGetFillLightBright(void)
{
	u16 ReadDat;
  STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_BRIGHT,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}

void FlashSetFillLightOn(uint16_t data)
{
	u16 ReadDat = data;
	STMFLASH_Write(FLASH_USE_FLASH_FILLLIGHT_ON,&ReadDat,1);
}

u16 FlashGetFillLightOn(void)
{
	u16 ReadDat = 0;
	STMFLASH_Read(FLASH_USE_FLASH_FILLLIGHT_ON,&ReadDat,1);
	return ReadDat == 65535 ? 0 : ReadDat;
}
