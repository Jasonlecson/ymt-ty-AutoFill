...
typedef struct sensor_control_struct
{
	...
	//补光灯
	char color_selection; //0:白色 1:红白 2:红色
	char color_selection_copy;
	char time_selection;	//0:常亮 1:8小时 2:12H小时 3:16小时
	char time_selection_copy;
	char control_mode;	//0:自动 1:手动
	char control_mode_copy;
	char brightness; //当前亮度（0-100%）
	char brightness_copy;
	char habit; //0:喜阳 1:喜阴
	char habit_copy;
	char openmode; //开灯模式
	char openmode_copy;
