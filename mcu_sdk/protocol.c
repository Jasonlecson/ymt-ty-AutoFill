...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_LIGHT_LOW_LIMIT, DP_TYPE_VALUE},
	{DPID_FILL_LIGHT_MODE, DP_TYPE_ENUM}, //122 自动开灯
	{DPID_FILL_LIGHT_ON, DP_TYPE_BOOL}, //123灯光开启
	{DPID_FILL_COLOR, DP_TYPE_ENUM}, //124 //灯光色温
	{DPID_FILL_TIMER, DP_TYPE_ENUM}, //125 //定时功能
	{DPID_FILL_BRIGHT_MODE, DP_TYPE_ENUM}, //126 //调光模式
	{DPID_FILL_BRIGHT, DP_TYPE_VALUE}, //127 //亮度
	{DPID_FILL_HABIT, DP_TYPE_ENUM} //128 //喜好
};
...
//	mcu_dp_value_update(DPID_WATER_COUNT,sensor_control_struct_value.watering_count);
	...
	//当前补光灯
	mcu_dp_enum_update(DPID_FILL_LIGHT_MODE, sensor_control_struct_value.openmode);//122
	mcu_dp_bool_update(DPID_FILL_LIGHT_ON,sensor_control_struct_value.fill_light_on); //123
	mcu_dp_enum_update(DPID_FILL_COLOR, sensor_control_struct_value.color_selection);//124
	mcu_dp_enum_update(DPID_FILL_TIMER, sensor_control_struct_value.time_selection);//125
	mcu_dp_enum_update(DPID_FILL_BRIGHT_MODE, sensor_control_struct_value.control_mode);//126
	mcu_dp_value_update(DPID_FILL_BRIGHT, sensor_control_struct_value.brightness);//127
	mcu_dp_enum_update(DPID_FILL_HABIT, sensor_control_struct_value.habit);//128
    /*
...
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);
    switch(on) {
        case 0:
			...
			FlashSetFillLightOn(sensor_control_struct_value.fill_light_on);
				mcu_dp_bool_update(DPID_FILL_LIGHT_ON,sensor_control_struct_value.fill_light_on);
...
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);
    switch(fill_light_mode) {
        case 0:
					...
					sensor_control_struct_value.openmode=0;
					sensor_control_struct_value.openmode_copy=0;
        break;
        
        case 1:
					FlashSetFillLightMode(1);
					sensor_control_struct_value.openmode=1;
...
功能描述 : 针对DPID_FILL_LIGHT_ON的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
//		printf("\r\nfill_light=%d\r\n",fill_light);
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_FILL_LIGHT_ON,fill_light);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fill_light_color_handle
功能描述 : 针对DPID_FILL_COLOR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fill_light_color_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char fill_light_color;
    
    fill_light_color = mcu_get_dp_download_enum(value,length);
	
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);
    switch(fill_light_color) {
        case 0:
					sensor_control_struct_value.color_selection=0;
					sensor_control_struct_value.color_selection_copy=0;
				FlashSetFillLightColor(0);
        break;
        
        case 1:
					sensor_control_struct_value.color_selection=1;
					sensor_control_struct_value.color_selection_copy=1;
				FlashSetFillLightColor(1);
        break;
				case 2:
					sensor_control_struct_value.color_selection=2;
					sensor_control_struct_value.color_selection_copy=2;
				FlashSetFillLightColor(2);
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_FILL_COLOR, fill_light_color);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fill_light_timer_handle
功能描述 : 针对DPID_FILL_TIMER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fill_light_timer_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char fill_light_timer;
    
    fill_light_timer = mcu_get_dp_download_enum(value,length);
	
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);

			sensor_control_struct_value.time_selection=fill_light_timer;
			sensor_control_struct_value.time_selection_copy=fill_light_timer;
			FlashSetFillLightTimer(sensor_control_struct_value.time_selection);
	time_flag = 1;
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_FILL_TIMER, fill_light_timer);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fill_light_bright_mode_handle
功能描述 : 针对DPID_FILL_BRIGHT_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fill_light_bright_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char fill_light_bright_mode;
    
    fill_light_bright_mode = mcu_get_dp_download_enum(value,length);
	
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);

			sensor_control_struct_value.control_mode=fill_light_bright_mode;
			sensor_control_struct_value.control_mode_copy=fill_light_bright_mode;
			FlashSetFillLightCtrlMode(sensor_control_struct_value.control_mode);

    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_FILL_BRIGHT_MODE, fill_light_bright_mode);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fill_light_bright_handle
功能描述 : 针对DPID_FILL_BRIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fill_light_bright_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long fill_light_bright;
    
    fill_light_bright = mcu_get_dp_download_value(value,length);
	
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);

			sensor_control_struct_value.brightness=fill_light_bright;
			sensor_control_struct_value.brightness_copy=fill_light_bright;
			FlashSetFillLightBright(sensor_control_struct_value.brightness);

    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_FILL_BRIGHT, fill_light_bright);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fill_light_habit_handle
功能描述 : 针对DPID_FILL_HABIT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fill_light_habit_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char fill_light_habit;
    
    fill_light_habit = mcu_get_dp_download_enum(value,length);
	
//		printf("\r\nfill_light_mode=%d\r\n",fill_light_mode);

			sensor_control_struct_value.habit=fill_light_habit;
			sensor_control_struct_value.habit_copy=fill_light_habit;
	FlashSetFillLightHabit(sensor_control_struct_value.habit);

    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_FILL_HABIT, fill_light_habit);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
				case DPID_FILL_LIGHT_MODE: //122
            //补光灯开启模式处理函数
            ret = dp_download_fill_light_mode_handle(value,length);
        break;
        case DPID_FILL_LIGHT_ON: //123
            //补光灯处理函数
            ret = dp_download_fill_light_handle(value,length);
        break;
				case DPID_FILL_COLOR: //124
						ret = dp_download_fill_light_color_handle(value,length);
				break;
				case DPID_FILL_TIMER: //125
						ret = dp_download_fill_light_timer_handle(value,length);
				break;
				case DPID_FILL_BRIGHT_MODE: //126
						ret = dp_download_fill_light_bright_mode_handle(value, length);
				break;
				case DPID_FILL_BRIGHT: //127
						ret = dp_download_fill_light_bright_handle(value, length);
				break;
				case DPID_FILL_HABIT : //128
						ret = dp_download_fill_light_habit_handle(value, length);
				break;
